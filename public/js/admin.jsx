...
function Chapitre({id, number, title, moveChapter, deleteChapter, experience}){
    return(
        <div className="chapter">
            <span id="left">
                ...
                <span id="content">Chapitre {number} - {title}</span>
            </span>
            <span id="right">
...
function Experience({experience, moveChapter, deleteChapter}){
    ...
    experience.chapters.forEach(chapter => {
        rows.push(<Chapitre key={chapter.id} id={chapter.id} number={chapter.number} title={chapter.title} moveChapter={moveChapter} deleteChapter={deleteChapter} experience={experience}/>)
...
class Admin extends React.Component{
    ...
    deleteChapter(id){
        if (window.confirm("Voulez vous vraiment supprimer la chapitre d'id "+id)){
    ...
    render(){
        ...
        this.state.experiences.forEach(experience => {
            rows.push(<Experience key={experience.id} experience={experience} moveChapter={this.moveChapter} deleteChapter={this.deleteChapter}/>)
        });
        return(
            <div>
                <h1>ADMIN PANEL</h1>
